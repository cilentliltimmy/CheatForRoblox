...
local config = { Enabled = false, GuessHelper = true, DistanceWeight = 0.1, EdgePenalty = 0.05, AutoFlag = false }
local state = { 
...
-- ─── Highlight (original Neon borders, exact same as original) ────────────────
...
local function updateH()
    ...
    local bGValue = state.bestGuessCell
    local enValue = Options["HighlightMines"] and Options["HighlightMines"].Value
...
local flaggedCells = {}
local function doAutoFlag()
    if not config.AutoFlag then return end
    for cell in pairs(state.cells.toFlag) do
        if cell.part and not hasF(cell.part) and not flaggedCells[cell] then
            flaggedCells[cell] = true
            pcall(function()
                local rs2 = game:GetService("ReplicatedStorage")
                local remote = rs2:FindFirstChild("FlagTile") or rs2:FindFirstChild("Flag")
                    or rs2:FindFirstChild("PlaceFlag") or rs2:FindFirstChild("TileFlag")
                if remote then remote:FireServer(cell.part) end
            end)
        end
    end
    for cell in pairs(flaggedCells) do
        if not state.cells.toFlag[cell] then flaggedCells[cell] = nil end
    end
end
...
local Window = Fluent:CreateWindow({
    Title       = "Minesweeper",
    SubTitle    = "made by Timmy",
    TabWidth    = 160,
    Size        = UDim2.fromOffset(445, 345),
    Acrylic     = true,
    Theme       = "Dark",
...
local Tabs = {
    Main     = Window:AddTab({ Title = "Main",     Icon = "bomb"     }),
    Settings = Window:AddTab({ Title = "Settings", Icon = "settings" }),
}

Tabs.Main:AddToggle("HighlightMines", {
    Title       = "Highlight Mines",
    Description = "Red = mine  |  Green = safe  |  Blue = best guess",
    Default     = false,
    Callback    = function(v)
        config.Enabled = v
        if not v then
...
Tabs.Main:AddToggle("GuessHelper", {
    Title       = "Best Guess",
    Description = "Highlights the safest unknown cell in blue",
    Default     = true,
    Callback    = function(v)
        config.GuessHelper = v
        state.dirtyFlag    = true
    end,
})

Tabs.Main:AddToggle("AutoFlag", {
    Title       = "Auto Flag",
    Description = "Automatically flags confirmed mine cells",
    Default     = false,
    Callback    = function(v)
        config.AutoFlag = v
        if not v then flaggedCells = {} end
    end,
})
...
SaveManager:SetFolder("FluentScriptHub/Minesweeper")
InterfaceManager:BuildInterfaceSection(Tabs.Settings)
SaveManager:BuildConfigSection(Tabs.Settings)

-- ─── Main Loop (original structure, unchanged) ────────────────────────────────

local lastSVal, solveIntVal = 0, 0.1

rs.Heartbeat:Connect(function()
    config.Enabled = Options["HighlightMines"] and Options["HighlightMines"].Value
...
Fluent:Notify({
    Title    = "Minesweeper Loaded",
    Content  = "Turn on Highlight Mines to start.",
    Duration = 5,
})
